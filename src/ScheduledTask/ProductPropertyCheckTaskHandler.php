...
class ProductPropertyCheckTaskHandler extends ScheduledTaskHandler
{
    ...
    public function run(): void
    {
        ...
        $this->logger->info('[CustomPropertyMapper] Scheduled Task STARTED: ' . $task::getTaskName());
        ...
        if ($task instanceof FrequentPropertyCheckTask) {
            ...
            $this->logger->info('[CustomPropertyMapper] Last run: ' . $lastRun);
        ...
        $this->logger->info('[CustomPropertyMapper] Products found: ' . $products->count());

        foreach ($products as $product) {
            $this->logger->info('[CustomPropertyMapper] Checking product ID: ' . $product->getId());

            $optionIds = $product->getProperties()->getIds();
            foreach ($optionIds as $optionId) {
                $this->logger->info('[CustomPropertyMapper] - Current property option: ' . $optionId);
            }
            ...
            foreach (self::PROPERTY_MAPPING as $sourceId => $targetId) {
                if (in_array($sourceId, $optionIds, true)) {
                    $this->logger->info("[CustomPropertyMapper] → Match found: $sourceId → $targetId");
            ...
            if ($updatedOptionIds !== $optionIds) {
                $this->logger->info("[CustomPropertyMapper] → Updating product: " . $product->getId());
                ...
                $this->logger->info("[CustomPropertyMapper] → No changes for product: " . $product->getId());
            }
        }

        $this->logger->info('[CustomPropertyMapper] Scheduled Task FINISHED');
    }
